...
impl Handler for AxumProxy {
    async fn run(&self, mut conn: Conn) -> Conn {
        ...
        // Preserve the original Host so Axum middleware (e.g. asset serving)
        // can distinguish virtual hosts behind this proxy.
        if let Some(host) = conn.request_headers().get_str("host") {
