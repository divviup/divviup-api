...
mod index {
    ...
    async fn as_member(app: DivviupApi) -> TestResult {
        ...
        assert_ok!(conn);
        let accounts: Vec<Account> = conn.response_json();
...
mod show {
    ...
    async fn as_a_member(app: DivviupApi) -> TestResult {
        let (user, account, ..) = fixtures::member(&app).await;
        let conn = get(format!("/api/accounts/{}", account.id))
            .with_api_headers()
            .with_state(user)
            .run_async(&app)
            .await;
        ...
        assert_ok!(conn);
        let account_response: Account = conn.response_json();
    ...
    async fn not_as_a_member_but_as_an_admin(app: DivviupApi) -> TestResult {
        ...
        assert_ok!(conn);
        let account: Account = conn.response_json();
    ...
    async fn as_admin_token(app: DivviupApi) -> TestResult {
        ...
        let conn = get(format!("/api/accounts/{}", other_account.id))
            ...
            .run_async(&app)
            .await;
        let response: Account = conn.response_json();
...
mod update {
    ...
    async fn as_a_member(app: DivviupApi) -> TestResult {
        ...
        let conn = patch(format!("/api/accounts/{}", account.id))
            .with_api_headers()
